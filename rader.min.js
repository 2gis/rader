...
p(L[b]).addClass(f.pointInRangeCls):p(L[b]).removeClass(f.pointInRangeCls)}a={};a[r.start]=C(e)+"%";a[r.size]=E(e)-C(e)+"%";p(U).css(a);if(f.onUpdate)f.onUpdate({minPos:C(e),maxPos:E(e),minVal:t(C(e)),maxVal:t(E(e))})}function V(a,d){if(a){for(var b=(a.clientX-W)*(100/z),b=n(h[s])+b,c=0;c<e.length;c++)Q[c]=e[c];J(s,s,F(b))}a:if(e.length!=Q.length)b=!1;else{for(b=0;b<e.length;b++)if(e[b]!==Q[b]){b=!1;break a}b=!0}b&&!d||K(d)}function X(){var a=C(e),d=E(e),b=N(t(a)),c=N(t(d));f.collapseVals&&d-a<100*
f.bumpRadius/z+1E-5&&(100==d?b=c:0==a?c=b:e[s]===a?c=b:b=c);return{minPos:a,maxPos:d,minVal:b,maxVal:c}}function Y(){f.move&&f.move(X())}function Z(){z=P[r.a];G=[];H=[];var a,d;for(d=0;d<g.length;d++)a={},a[r.start]=n(g[d])+"%",p(L[d]).css(a);for(d=0;d<h.length;d++)a={},e[d]=n(h[d]),a[r.start]=e[d]+"%",p(D[d]).css(a);d=n(h[0]);var b=n(h[h.length-1]);a={};a[r.start]=d+"%";a[r.size]=Math.abs(b-d)+"%";p(U).css(a)}var M,z,r={start:"left",a:"clientWidth",offset:"offsetWidth",size:"width"},l,p,W,s=-1,h=
[],e=[],Q=[],A=[];this.elements={};p=f.dom||q;var P=f.root[0],U=f.trackActive,L=f.points,D=f.runners;l={stickingRadius:0,bumpRadius:D[0][r.offset],points:[],pointsPos:[0,10],runnersPos:[]};for(var v in l)f[v]===x&&(f[v]=l[v]);var g=f.pointsPos,w=f.runnersPos;v=f.runnersVal;var y=f.start,I=f.end,k=f.values,y=g[0],I=g[g.length-1];k||(k=g);if(v&&!w.length)for(l=0;l<v.length;l++)w[l]=R(v[l]);w.length||(w=[y,I]);M=Math.abs(I-y);var G=[],H=[],O;for(l=0;l<w.length;l++)q(D[l]).on("mousedown.rader",function(a){return function(d){2!=
d.button&&(d.preventDefault(),B(),s=a)}}(l));(function(){h=w.slice();for(var a=0;a<w.length;a++)e[a]=n(h[a])})();Z();V(0,1);q(document).on("mouseup.rader blur.rader",function(){if(-1!=s){for(var a=0,d=h.length;a<d;a++)h[a]=t(e[a]);f.change&&f.change(X());B(1)}s=-1});q(document).on("mousedown.rader",function(a){W=a.clientX});q(document).on("mousemove.rader",function(a){-1!=s&&(V(a,0),Y())});this.setPosition=function(a,d){var b=n(d);J(a,a,b);for(b=0;b<e.length;b++)h[b]=t(e[b]);K(1)};this.setValue=function(a,
